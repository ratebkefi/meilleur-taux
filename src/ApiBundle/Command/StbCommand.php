...
class StbCommand extends ContainerAwareCommand
{
    private $output;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $html = @file_get_contents("http://www.stb.com.tn/fr/site/bourse-change/cours-de-change/"); //getting the file content


            $banque = $repository->getRepository('ApiBundle:Banque')->findOneBy(array('raison_social' => 'STB'));
        if(!$html){
            $logs = new Logs();
        ...
        //$html = file_get_contents("http://www.stb.com.tn/fr/site/bourse-change/cours-de-change/");
        $crawler = new Crawler($html);
        $link = $crawler->filter('table > tr > td ');
        $i=0;
        $j=0;
        $s=0;
        $p=0;
