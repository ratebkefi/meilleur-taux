...
class PosteCommand extends ContainerAwareCommand
{
    private $output;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $html = @file_get_contents("http://www.poste.tn/change.php"); //getting the file content
        $banque = $repository->getRepository('ApiBundle:Banque')->findOneBy(array('raison_social' => 'La poste'));
        if(!$html){
            $logs = new Logs();
        ...
        //$html = file_get_contents("https://www.poste.tn/change.php");
        $crawler = new Crawler($html);
        $link = $crawler->filter('td.cel_contenu');
        $i=0;
        $j=0;
        $array_receive= array();
        ...
        foreach($array_receive as $adresse){
            ...
            $repository = $this->getContainer()->get('doctrine.odm.mongodb.document_manager');
            $banque = $repository->getRepository('ApiBundle:Devise')->findOneBy(array('code_iso' => $adresse["code"]));
            $taux->setDevise($banque);

            $repository = $this->getContainer()->get('doctrine.odm.mongodb.document_manager');
            $devise = $repository->getRepository('ApiBundle:Banque')->findOneBy(array('raison_social' => 'La poste'));
